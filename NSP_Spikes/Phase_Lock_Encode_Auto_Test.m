%% Phase Lock Automated Encoding - Pranish Kantak Last Updated - 10/28/2018
...
directories and filter event structure for successfully encoded words.
...
for subjdir = root_dir'
    ...
    % SME is recEv vs. nonrecEv
    recEv = filterStruct(events,'recalled == 1');
    nonrecEv = filterStruct(events,'recalled == 0');
    ...
    for pos_or_neg=[pos_files;neg_files]'
        ...
        for current_lfp=lfp_files'
            ...
            if any(cluster_class(:,1)) == 1
                ...
                % grab spikes that occur 1800 ms after each successful recall event
                for n = 1:length(recEv)
                    
                    spikesIwant = ((cluster_class(:,2) >= ((recEv(n).eegoffset)) & (cluster_class(:,2) <= ((recEv(n).eegoffset) + 1800))));
                    cluster_class_recalled = cluster_class(spikesIwant,:);
                    
                    cluster_class_recalled_cat = [cluster_class_recalled_cat;cluster_class_recalled];
                    
                end
                
                % round spikeTimes in cluster_class_recalled to the nearest .5 millisecond
                tmp = cluster_class_recalled_cat(:,1);
                tmp1 = round((cluster_class_recalled_cat(:,2))*2)/2;
                cluster_class_recalled = cat(2,tmp,tmp1);
                clear tmp tmp1
                ...
                for i = clusters
                    ...
                    temp = cluster_class_recalled(:,1) == i;
                    cluster_split{i} = double(cluster_class_recalled(temp,:));
                    if i >= 2
                ...
                clear tmp tmp1
                % convert spikeTimes in cluster_class_recalled to seconds; then convert to sample number
                cluster_split_recalled_samples = {};
                for i = clusters
                    tmp{i} = cluster_split{:,i}(:,1);
                    tmp2{i} = ((cluster_split{:,i}(:,2)) * 2);
                    cluster_split_recalled_samples{i} = cat(2,tmp{i},tmp2{i});
                    cluster_split_recalled_samples{i} = round(cluster_split_recalled_samples{:,i});
                    
                    if i >= 2
                        if isempty(cluster_split_recalled_samples{i-1})
                            cluster_split_recalled_samples{i-1} = nan(1,2);
                        end
                    end
                end
                ...
                for i = clusters
                    ...
                    if all(isnan(cluster_split_recalled_samples{1,i}))
                        continue
                    else
                        
                        Bandphase_logical(i,cluster_split_recalled_samples{:,i}(:,2)) = true;
                    end
                ...
                if any(strfind(pos_or_neg.folder,'Pos'))
                    savename = fullfile(sprintf('/Users/Pranish/Desktop/FR1_Local_Data/Have_Events/%s/Analysis_Encode/Pos/%s_%s.mat',subject, strrep(strrep(pos_or_neg.name, '.mat',''),'times_',''), strrep(strrep(current_lfp.name, '.mat',''),'Modal_','')));
                elseif any(strfind(pos_or_neg.folder,'Neg'))
                    savename = fullfile(sprintf('/Users/Pranish/Desktop/FR1_Local_Data/Have_Events/%s/Analysis_Encode/Neg/%s_%s.mat',subject, strrep(strrep(pos_or_neg.name, '.mat',''),'times_',''), strrep(strrep(current_lfp.name, '.mat',''),'Modal_','')));
                end
                
                
                if ~isempty(phases) == 1
                    save(savename,'pval','z','mu','ul','ll','bands','phases','frequency_sliding','pos_or_neg','current_lfp','clusters');
                    clearvars -except cluster_class_recalled_cat current_dir subject lfp_files pos_files neg_files events recEv nonrecEv pos_or_neg current_lfp cluster_class;
