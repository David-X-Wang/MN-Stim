...
for ses = 1:length(session_paths)
    ...
    for j = 1:length(NS6_idx)
        ...
        try
            cd('sort_neg_simple') % what bout positivE?   'pos'
        catch
            continue
        end
        try
        ...
        for t = 1:length(orig_data.GroupHierarchy.Groups(1).Datasets) % remember, 1 is negative, 2 is positive
            struct_names{t} = [orig_data.GroupHierarchy.Groups(1).Datasets(t).Name];
        end
        
        Times = find(strcmp(struct_names,'/neg/times')); % what if they are positive?     'pos'
        Waveforms = find(strcmp(struct_names,'/neg/spikes'));
        
        spikeWaveforms = hdf5read(orig_data.GroupHierarchy.Groups(1).Datasets(Waveforms));% remember, 1 is negative, 2 is positive
        spikeTimes = hdf5read(orig_data.GroupHierarchy.Groups(1).Datasets(Times));
