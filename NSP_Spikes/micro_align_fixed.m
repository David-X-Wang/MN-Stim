function events = micro_align_fixed(subj,raw_name,sessnum,sync_ch)
%
% FUNCTION:
...
basedir = '/Volumes/Pranish/FR1_Local_Data';

fix_align = 'fixed_for_alignment/NS3';

rawdir = sprintf('%s/%s/raw/%s/%s',basedir,subj,raw_name,fix_align); %pk added the last %s for the "fixed_for_alignment" folder
...
eegfile = [rawdir]; % pk deleted '/NS3' because it is already in the rawdir name
